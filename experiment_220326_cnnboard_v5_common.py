...
def get_transposer(config):
    if config == 'top-left':
        return lambda model, input, d: model(torch.from_numpy(input).unsqueeze(0).to(d, torch.float32)).argmax().item()
...
def transposer_lr(model, inp, d):
    ...
    action = model(torch.from_numpy(input_aug).unsqueeze(0).to(d, torch.float32)).argmax().item()
    new_action_lr = None
    if action == 1: # Right
