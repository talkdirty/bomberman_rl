...
def gym_field_agents(field, the_other_states):
    f = np.zeros_like(field, dtype=np.float32)
    base_multiplier = .33
    for ix, s in enumerate(the_other_states):
        x, y = s['self'][3]
        f[y, x] = (ix+1) * base_multiplier
        for other in s['others']:
            x, y = other[3]
            f[y, x] = -1. * (ix+1) * base_multiplier
    return f

    
def state_to_gym(game_state: dict, the_other_states) -> Union[dict, None]:
    """
    ...
    agent_field = gym_field_agents(game_state['field'].T, the_other_states)
